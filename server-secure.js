...
const COUCHDB_DATABASE = process.env.COUCHDB_DATABASE || 'maia_chats';
...
const initializeCouchDB = async () => {
  try {
    ...
    if (response.ok) {
      ...
      throw new Error(`CouchDB connection failed: ${response.status}`);
    }
    
    // Initialize database if it doesn't exist
    try {
      const createResponse = await fetch(`${COUCHDB_URL}/${COUCHDB_DATABASE}`, {
        method: 'PUT',
        headers: {
          'Authorization': couchAuthHeader,
      ...
      if (createResponse.ok) {
        console.log(`✅ Database '${COUCHDB_DATABASE}' created`);
      } else if (createResponse.status === 412) {
        console.log(`✅ Database '${COUCHDB_DATABASE}' already exists`);
      } else {
        console.error('❌ CouchDB database creation failed:', createResponse.status);
      }
    } catch (err) {
      console.error('❌ CouchDB database creation failed:', err);
    }
    
    console.log(`✅ Using database '${COUCHDB_DATABASE}'`);
...
app.post('/api/save-chat', async (req, res) => {
  try {
    ...
    // Use direct HTTP request instead of nano
    const response = await fetch(`${COUCHDB_URL}/${COUCHDB_DATABASE}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': couchAuthHeader
    ...
    console.log(`💾 Chat saved to CouchDB: ${result._id}`);
...
app.get('/api/load-chats/:patientId?', async (req, res) => {
  try {
    ...
    // Use direct HTTP request instead of nano
    const response = await fetch(`${COUCHDB_URL}/${COUCHDB_DATABASE}/_all_docs?include_docs=true`, {
      headers: {
        'Authorization': couchAuthHeader
      }
    });
    ...
    const chats = result.rows
      .filter(row => row.doc && row.doc.patientId === patientId)
      .map(row => ({
        id: row.doc._id,
        patientId: row.doc.patientId,
        createdAt: row.doc.createdAt,
        updatedAt: row.doc.updatedAt,
        participantCount: row.doc.participantCount,
        messageCount: row.doc.messageCount,
        chatHistory: row.doc.chatHistory,
        uploadedFiles: row.doc.uploadedFiles || []
      }))
...
app.get('/api/load-chat/:chatId', async (req, res) => {
  try {
    ...
    // Use direct HTTP request instead of nano
    const response = await fetch(`${COUCHDB_URL}/${COUCHDB_DATABASE}/${chatId}`, {
      headers: {
        'Authorization': couchAuthHeader
      }
    });
...
app.delete('/api/delete-chat/:chatId', async (req, res) => {
  try {
    ...
    const getResponse = await fetch(`${COUCHDB_URL}/${COUCHDB_DATABASE}/${chatId}`, {
      headers: {
        'Authorization': couchAuthHeader
      }
    });
    ...
    const chat = await getResponse.json();
    
    // Delete the document
    const deleteResponse = await fetch(`${COUCHDB_URL}/${COUCHDB_DATABASE}/${chatId}?rev=${chat._rev}`, {
      method: 'DELETE',
      headers: {
        'Authorization': couchAuthHeader
      }
